...
def get_pie_chart(entered_site):
    if entered_site == 'ALL':
        fig = px.pie(values=spacex_df['class'].value_counts(),
                     names=['Success', 'Failure'],
                     title='All Launch Outcomes')
        return fig
    else:
        filtered_df = spacex_df[spacex_df['Launch Site'] == entered_site]  # Column name: 'Launch Site'
...
def get_scatter_chart(entered_site, entered_payload):
    if entered_site == 'ALL':
        ...
        fig = px.scatter(filtered_df,
                        x='Payload Mass (kg)',
                        y='class',
                        color='Booster Version Category',  # Exact column name
